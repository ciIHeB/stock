    ...
    <div>
        <div class="panel px-0 pb-1.5 border-[#e0e6ed] dark:border-[#1b2e4b]">
            <div class="datatable invoice-table">
                ...
                >
                <template #UserIsActive="data">
                            <span :class="getActiveClass(data.value.UserIsActive)">
                                {{ data.value.UserIsActive ? 'Active' : 'Inactive' }}
                            </span>
                        </template>
                ...
                <template #actions="data">
                        <div class="flex gap-4 items-center justify-center">
                            ...
                            <button type="button" class="hover:text-danger" @click="deleteRow(`${data.value.UserID}`)"> 
...
<script lang="ts" setup>
    ...
    try {
      ...
      const response = await userService.createItem(ItemData);
  ...
  const edit = async (id) => {
   try {
        ...
        console.log("id", id);
      const response = await userService.updateItem(id,updatedData);
  ...
  //----------------------
    ...
    const cols = ref([
        ...
        { field: 'UserIsActive', title: 'Status', template: 'IsActive' },
   ...
   const getActiveClass = (isActive) => {
    ...
    try {
        const itemsData = await userService.getItems();
        ...
        const resolveduserGroup= await Promise.all(userGroupPromises);
            ...
            items.value = itemsData.map((itemsData , index)=> ({
                ...
                LastUpdateDate:formatDate(itemsData.LastUpdateDate),
...
const fetchUserGroup= async (UserGroupID) => {
    try {
        const userGroupData = await secUserGroupService.getItemById(UserGroupID);
        return userGroupData.UserGroupNameLan1;
    } catch (error) {
...
async function  fetchUsersGroup ()  {
    try {
      groups.value = await secUserGroupService.getItems();
