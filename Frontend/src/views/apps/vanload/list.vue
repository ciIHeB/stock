            ...
            <div class="datatable invoice-table">
                ...
                <template #actions="data">
                        <div class="flex gap-4 items-center justify-center">
                            <router-link :to="`/apps/vload/approve/${data.value.SRNumber}`"  class="hover:text-info">
...
<script lang="ts" setup>
    ...
    try {
        const itemsData = await invAutoSRHeaderService.getloadItems();

        
        const companyPromises = itemsData.map(itemsData => {
                    return fetchCompany(itemsData.CompanyID);
                });
        const ownerPromises = itemsData.map(itemsData => {
                    return fetchOwners(itemsData.OwnerID);
                });
        const branchPromises = itemsData.map(itemsData => {
                    return fetchBranchByOwner(itemsData.OwnerID);
                });
        ...
        const resolvedBranches = await Promise.all(branchPromises);
            ...
            items.value = itemsData.map((itemsData , index)=> ({
                ...
                Owner: resolvedOwners[index],
                Branch: resolvedBranches[index],
                AutoSRDate:formatDate(itemsData.AutoSRDate)
            }));
...
const fetchBranchByOwner = async (OwnerID) => {
  try {
    const owner = await ownerService.getOwnerById(OwnerID);
    if (!owner.BranchID) return "—";

    const branch = await branchService.getBranchById(owner.BranchID);
    return branch.BranchDescLan1;
  } catch (error) {
    console.error('Error fetching Branch by Owner:', error);
    return "—";
  }
};
