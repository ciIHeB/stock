<template>
  <form @submit.prevent="handleSubmit" class="mt-8 px-4 w-full">
    ...
    <button type="submit" class="btn btn btn-primary mt-6 float-right">Create Customer</button>
  </form>
</template>
...
try {
  ...
  const itemData = {
    ...
    CustomerCreationDate: new Date().toISOString(),
    CustomerLastUpdateTime: new Date().toISOString()
  };

  // Save item data
  const response = await customerService.createItem(itemData);
