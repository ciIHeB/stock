<template>
  <form @submit.prevent="handleSubmit" class="mt-8 px-4 w-full">
    ...
    <button type="submit" class="btn btn btn-primary mt-6 float-right">Update Owner</button>
  </form>
</template>
...
<script lang="ts" setup>
  ...
  import hhuser from '@/services/hhuserService';
  ...
  const owner_id = route.params.id;
  ...
  const fetchOwnerDetails = async (id: any) => {
    try {
        const OwnerDetails = await ownerService.getOwnerById(id);
       const UserDetails = await hhuser.getUserById(OwnerDetails.UserID);

        //formData.value = OwnerDetails; // Update formData value
        formData.value.companyID=OwnerDetails.CompanyID,
        formData.value.HHUserName=UserDetails.HHUserName,
        formData.value.BranchID=OwnerDetails.BranchID,
        formData.value.SupervisorID=OwnerDetails.SupervisorID,
        formData.value.OwnerDescLan1=OwnerDetails.OwnerDescLan1,
        formData.value.StoreID=OwnerDetails.StoreID,
        formData.value.OwnerAutoSynchMode=OwnerDetails.OwnerAutoSynchMode,
        formData.value.OwnerMaxStockVal=OwnerDetails.OwnerMaxStockVal,
        formData.value.OwnerIsActive=OwnerDetails.OwnerIsActive,
        formData.value.OwnerDefaultWHID=OwnerDetails.OwnerDefaultWHID,
        formData.value.OwnerCreditLimitValue=OwnerDetails.OwnerCreditLimitValue,
  ...
  try {
    const OwnerDetails = await ownerService.getOwnerById(owner_id);
    ...
    // Save item data
    const response = await ownerService.updateOwner(owner_id,itemData);
